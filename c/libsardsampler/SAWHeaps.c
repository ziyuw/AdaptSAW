...
void removeFromSAWHeap( pSAWHeap p_saw_heap, int saw_heap_array_ind )
{
  /* Function to remove element saw_heap_array_el from the heap array.
     ...
     Elegant way it works (stole the idea from various places on the
     web:)
     
     1) Set the key of the target element to Inf, then bubble it up to
     the root.
...
int sampleFromSAWHeap( pSAWHeap p_saw_heap )
{
  /* Function to sample an element from the SAW heap based on the
     weights at the elements. Undefined behaviour if you try on an
     empty heap.

     Returned: the position in the SAWHeapArray of the sampled
     element. NOT the graph node! You can recover all the necessary
     info such as the node in the graph it corresponded to by
     accessing the fields of that entry of the array.

     Works by refining the interval that random variable R could have
     been in. Pretty straightforward.

     Note: I could have made it conceptually more simple by repeatedly
     drawing random samples to choose a path along the heap, but I did
     it this way for the challenge. Only one random sample is drawn
  ...
  */
  ...
  while ( (2*curr_element+1) < num_elements ) {
    ...
    /* If the weight of the curr_el has won: R in [interval_start, W_e+interval_start] */
    /* Reminder: R has been scaled!! */
    if ( ( R > interval_start )     &&    ( R <= interval_start+W_e ) ) {
      break;
    } else if ( ( R > interval_start+W_e )   &&  ( R<= interval_start+W_e+S_L ) ) { /* weight of the left child has won: */

      interval_start += W_e;
      curr_element = left_child;
    } else { /* weight of the right child has won: */
