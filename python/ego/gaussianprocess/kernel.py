...
class Kernel(object):
    ...
    def derivative_partial(self, X, x):
	raise NotImplementedError('kernel-derived class does not have derivative_partial method')
...
class GaussianKernel_ard(Kernel):
    ...
    def derivative_partial(self, X, x):
	"""
	Computes the derivative for the covariance between x and X
	"""
	...
	for i in xrange(M):
	    C[i] = -self.cov(X[i], x)*(self._itheta2*(x-X[i]))
	    
	    #print 'start', X[i], x, self._itheta2*(X[i]-x), C[i], self.cov(X[i], x)
