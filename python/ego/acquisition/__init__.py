...
class UCB(object):
    ...
    def f(self, x, getGrad=False):
	"""
	If getGrad is true then return the negative of the ubc objective and gradient.
	"""
	
        if not getGrad:
	    return self.negf(x)
        else:
	    return self.negf_grad(x)

    def negf_grad(self, x):
	"""
	Returns the negative GP-mean at x as well as the negative gradient of GP-mean also at x.
	"""
	mu, sig2, grad_mean, grad_var = self.GP.posterior(x, True, True)
	
	beta = sqrt(self.scale * self.sBeta);
	sqrt_sig2 = sqrt(sig2)
        neg_ucb = -(mu + beta * sqrt_sig2)
	neg_ucb_grad = -(grad_mean + (beta*0.5/sqrt_sig2)*grad_var)
	
	return float(mean), meangrad
...
def maximizeMean(model, bounds, maxiter=50, maxtime=30, maxsample=10000):
    ...
    opt, optx = direct(gp_mean_obj, bounds, args=None, debug=False, maxiter=maxiter, maxtime=maxtime, maxsample=maxsample)
    return optx, opt

def maximizeUCB_local(model, bounds, x0):
    """
    Maximize the GP-mean by numerical method to find a local optima.
    Current implementation uses l-bfgs-b from scipy
    """
    
    def gp_mean_obj_grad(x, gp=model):
	mu, sig2, grad_mean, grad_var = gp.posterior(x, True, True)
	mean = -mu
	meangrad = -grad_mean
	return float(mean), meangrad
    ...
    x, f, d = scipy.optimize.fmin_l_bfgs_b(gp_mean_obj_grad, x0, bounds = bounds, approx_grad = False)
    return x, f
